...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/provordResp:ResponseMessage[provordResp:ResponseXML/provordResp:status/text() = 'SUCCESS']">
		...
		<OrderDataUpdate xmlns="http://www.metasolv.com/OMS/OrderDataUpdate/2002/10/25">
			...
			<Update path="/TaskName">TestSOMAutomationReceiverOnlyJMSCorrIDTask1</Update>
	...
	<xsl:template match="/provordResp:ResponseMessage[provordResp:ResponseXML/provordResp:status/text() = 'FAILURE']">
		<xsl:variable name="generallog"	select="java:info($log, concat('Hello ','is it me you are looking for'))" />
		<xsl:variable name="automator" select="java:setUpdateOrder($automator, true())" />
